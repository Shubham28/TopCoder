...
double FoxPlayingGame::theMax(int nA, int nB, int prA, int prB) {
	double scA=prA/1000.0,scB=prB/1000.0;

	double ans=0.0;
	if(scA>=0 && scB>=0){
		if(scB>=1)
			ans=(nA*scA)*pow(scB,nB);
		else
			ans=(nA*scA);
	} else if(scA>=0 && scB<=0){
		if(abs(scB)>=1.0)
			ans=(nA*scA)*pow(scB,(nB&1)?(nB-1):nB);
		else
			ans=(nA*scA);
	} else if(scA<=0 && scB>=0){
		if(abs(scB)>=1.0)
