...
string MatchNumbersEasy::maxNumber(vector <int> mat, int n) {
	VI done(n+1);
	VS comb(n+1,"");
	done[0]=n;
	...
	EFOR(r,0,n){
		FOR(m,0,mat.size())
			if(r-mat[m]>=0 && done[r-mat[m]]-mat[m]>=0){
				string ret=comb[r-mat[m]];
				if(!(ret=="0" && m==0))
					PB(ret,char(48+m));

				if(comb[r].size()<ret.size() || (comb[r].size()==ret.size() && comb[r]<ret) )
					comb[r]=ret,done[r]=done[r-mat[m]]-mat[m];
			}

	}

	string ans;
	RFOR(c,comb.size()-1,0)
		if(comb[c]!="" && comb[c].size()>ans.size() || (comb[c].size()==ans.size() && comb[c]>ans))
			ans=comb[c];
