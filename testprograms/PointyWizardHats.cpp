...
public:
	...
	bool canPlace(int tpH, int tpR, int btH, int btR){
		return (tpR<btR && btH*tpR<btR*tpH);
	}
...
int PointyWizardHats::getNumHats(vector <int> tpH, vector <int> tpR, vector <int> btH, vector <int> btR) {
	T=SZ(tpH),B=SZ(btH);

	FOR(i,0,T)		FOR(j,0,B)
		if(canPlace(tpH[i],tpR[i],btH[j],btR[j]))
			graph[i][j]=1;
