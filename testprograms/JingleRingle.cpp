...
int JingleRingle::profit(vector <int> bO, vector <int> sO, int tax) {
	double tx=tax;
	SORT(sO);
	SORT(bO);
	...
	int pr=0;
	FOR(ran,0,min(bO.size(),sO.size()))
		if((bO[ran]-sO[ran])-int(bO[ran]*tx/100.0+1e-7)>=0)
			pr+=(bO[ran]-sO[ran])-int(bO[ran]*tx/100.0+1e-7);
