...
int SRMRoomAssignmentPhase::countCompetitors(vector <int> rat, int K) {
	int ours=rat[0];
	SORT(rat);		reverse(ALL(rat));

	int pos=find(ALL(rat),ours)-rat.begin();
	int room=pos%K;
	int ans=0;
	for(int a=room;a<SZ(rat) && a<pos;a+=K,ans++);
	return ans;
}
